...
独角数卡 2.0.6（`assimon/dujiaoka`，提交 `e846d12`）的顾客页面和店主功能，用 Go 与 PostgreSQL 重新实现。代码新写，不复制它的 PHP、Blade、SQL 或邮件 HTML，仓库不附带它的 MIT 声明。做完后单独公开，仓库名 Dufaka-Go。

验收标准：前台三套皮肤、后台每个菜单、网址、字段、按钮、文案、订单状态和支付渠道，与独角数卡一致。不允许少一页、少一个字段、改一条成功路径。本文列出的优化只发生在服务器内部。
...
默认关闭。关闭时前台和后台与独角数卡一致，不出现新按钮。打开后只增加助手，不改购买、计价、锁卡和支付。
